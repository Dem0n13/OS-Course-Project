        ...
        public override string ToString()
        {
            ...
            switch (Type)
            {
                case RequestTypes.MemoryToMemory:
                    result = String.Format("Memory>Memory (А={0}_{1} > А={2}_{3})", FromTable, FromDescriptor, ToTable, ToDescriptor);
                    break;
                case RequestTypes.MemoryToHDD:
                    result = String.Format("Memory>HDD (А={0}_{1} > {2}, FB={3})", FromTable, FromDescriptor, ToFile, FileBlockNum);
                    break;
                case RequestTypes.HDDToMemory:
                    result = String.Format("HDD>Memory ({0}, FB={1} > А={2}_{3})", FromFile, FileBlockNum, ToTable, ToDescriptor);
                    break;
                case RequestTypes.Action:
                    result = String.Format("Data Edit (А={0}_{1})", FromTable, FromDescriptor);
                    break;
            }
            return result;
        }
