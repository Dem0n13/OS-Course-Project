        ...
        public override string ToString()
        {
            ...
            switch (Type)
            {
                case RequestTypes.MemoryToMemory:
                    result = String.Format("Заявка Память>Память (ЛА={0}_{1} > ЛА={2}_{3})", FromTable, FromDescriptor, ToTable, ToDescriptor);
                    break;
                case RequestTypes.MemoryToHDD:
                    result = String.Format("Заявка Память>ВЗУ (ЛА={0}_{1} > {2}, ФБ={3})", FromTable, FromDescriptor, ToFile, FileBlockNum);
                    break;
                case RequestTypes.HDDToMemory:
                    result = String.Format("Заявка ВЗУ>Память ({0}, ФБ={1} > ЛА={2}_{3})", FromFile, FileBlockNum, ToTable, ToDescriptor);
                    break;
                case RequestTypes.Action:
                    result = String.Format("Заявка Обработка данных (ЛА={0}_{1})", FromTable, FromDescriptor);
                    break;
            }
            return result;
        }
