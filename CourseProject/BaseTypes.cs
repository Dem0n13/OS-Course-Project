        ...
        public override string ToString()
        {
            ...
            switch (Type)
            {
                ...
                case RequestTypes.MemoryToHDD:
                    result = String.Format("Память > Жесткий (ЛА={0}_{1} > {2}, ФБ={3})", FromTable, FromDescriptor, ToFile, FileBlockNum);
                    break;
                case RequestTypes.HDDToMemory:
                    result = String.Format("Жесткий > Память ({0}, ФБ={1} > ЛА={2}_{3})", FromFile, FileBlockNum, ToTable, ToDescriptor);
                    break;
                case RequestTypes.Action:
                    result = String.Format("Действия (/2) (ЛА={0}_{1})", FromTable, FromDescriptor);
                    break;
            }
            return result;
        }
